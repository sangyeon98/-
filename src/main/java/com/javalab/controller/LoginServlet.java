...
public class LoginServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, 
		...
		System.out.println("추출 성공" + id + "이건 비번" +pwd);
		...
		MemberVo vo = new MemberVo(id, pwd);	// Dao에 인자로 보낼 객체 생성
		
		
		// 4 사용자 아이디/비밀번호 확인
		MemberVo voCheck = memberDao.loginCheck(vo);
		...
		if(voCheck == null) {
			...
			session.setAttribute("member", voCheck);
			...
			// member - id 쿠키 객체 생성
			Cookie cookie = new Cookie("id", voCheck.getId());
			response.addCookie(cookie);
			System.out.println("로그인 성공! 세션과 쿠키 생성 완료!");
			
			// 4.6 로그인 성공후 게시물 목록 페이지로 이동
			url = contextPath+"/main/boardList.jsp";
